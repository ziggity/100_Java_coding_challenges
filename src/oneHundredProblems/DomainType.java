...
public class DomainType {
	...
	System.out.println(domainType(domains));
	}

	private static String domainType(String[] domains) {
		List<String> domainType = new ArrayList<String>();
		for(int i = 0; i < domains.length; i++) {
			
			String[] domain = domains[i].split(".");
			String lastDomain = domain.toString();
			System.out.println(lastDomain.toString() +" last" );
			
			if(lastDomain == "org") {
				domainType.add("organization");
			}else if (lastDomain == "com") {
				domainType.add("commercial");
			}else if(lastDomain == "net") {
				domainType.add("network");
			} else if(lastDomain == "info") {
				domainType.add("information");
			}
		}
