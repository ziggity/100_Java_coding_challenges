...
public class FareEstimator {
	...
	public static void main(String[] args) {
		double[] costPerMin = {0.2, 0.35, 0.4, .45};
		double[] costPerMile = {1.1, 1.8, 2.3, 3.5};
		
		System.out.println(fareEstimator(30,7, costPerMin, costPerMile));
	}

	private static String fareEstimator(int ride_time, int rideDistance, double[] costPerMin, double[] costPerMile) {
		ArrayList fareCost = new ArrayList();
		for (int i = 0; i < costPerMile.length; i++) {
	        fareCost.add(ride_time * costPerMin[i] + rideDistance * costPerMile[i]);
	    }

		
		return fareCost.toString();
	}
