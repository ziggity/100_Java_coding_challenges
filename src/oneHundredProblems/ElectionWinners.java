...
public class ElectionWinners {
	...
	public static void main(String[] args) {
		Integer[] voters = {2, 3, 5, 2};
		int remainingVotes = 3;
		System.out.println(electionWinners(voters, remainingVotes));
	}

	private static int electionWinners(Integer[] voters, int remainingVotes) {
		int inTheRunning = 0;
		int mostVotes = Collections.max(Arrays.asList(voters));
		Arrays. sort(voters);
		if(voters[0] != voters[1] && remainingVotes == 0){
		    return 1;
		}
		for(int vote : voters) {
			if(mostVotes - vote < remainingVotes) {
				inTheRunning++;
			}
		}
