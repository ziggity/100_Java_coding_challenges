 ...
 */
public class CompanBot {
	public static void main(String[] args) {
		double[][] companyInput = {{3, 1}, {6, 1}, {4, 1}, {5, 1}};
		System.out.println(companyBot(companyInput));
	}

	private static  double companyBot(double[][] companyInput) {
		int time = 0;
		int correctness = 0;
		for(double[] data : companyInput) {
			if(data[1] == 1) {
				time += data[0];
				correctness += data[1]; 
			}
		}
		if(time / correctness >= 0) {
